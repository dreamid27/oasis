...
func TestRunIngestPipeline_CallsProcessorsInOrder(t *testing.T) {
	p1 := &captureProc{addCandidate: MemoryItem{ID: "a", Kind: KindFact, Content: "fact a"}}
	p2 := &captureProc{}
	ctx := &IngestContext{}
	err := runIngestPipeline(context.Background(), ctx, []IngestProcessor{p1, p2})
	if err != nil { t.Fatal(err) }
	if !p1.called || !p2.called { t.Fatal("processors not called") }
...
func TestRunIngestPipeline_StopsOnError(t *testing.T) {
	...
	if after.called { t.Fatal("processor after error should not run") }
}

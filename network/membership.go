...
func (n *Network) AddAgent(child core.Agent) error {
	...
	// Why: the runtime's non-dynamic ResolveTools path returns the cached
	// tool-def slice unchanged. Without this rebuild, the router LLM never
	// sees an `agent_<name>` definition for the newly added child and silently
	// can't delegate to it. Skipped when a DynamicTools resolver is set
	// because that path rebuilds per Execute.
...
func (n *Network) RemoveAgent(name string) error {
	...
	// Why: see AddAgent — keep the cached tool defs in sync with membership
	// so the router stops advertising a tool whose child is gone.
	if !n.HasDynamicTools() {
		n.rebuildCachedToolDefsLocked()
	}
	// Mark the dynamic-path cache stale so the next Execute rebuilds it.

...
func (c *AgentCore) Limits() Limits { return limitsFromConfig(&c.Config) }
...
//
// Why one `lc.Config = *cfg` instead of ~18 named field copies: the embedded
// Config in LoopConfig means every overridable knob promotes automatically,
// so adding a new knob doesn't require touching this function.
func (c *AgentCore) BaseLoopConfig(
	...
	return LoopConfig{
		...
		dispatch:     dispatch,
		systemPrompt: prompt,
		// maxSteps: dereferenced from *Config.maxSteps to avoid pointer indirection in the hot loop.
		maxSteps: maxSteps,

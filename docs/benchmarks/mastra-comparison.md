...
| Feature                         | Mastra                                                                                              | Oasis                                                                                  | Winner |
|---------------------------------|-----------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|--------|
| **Architecture**                | 3 named processors (`MessageHistory`, `WorkingMemory`, `SemanticRecall`) + `ObservationalMemory` engine (replaces `MessageHistory` rather than composing); `Memory` class 2,873 LOC + ObservationalMemory 3,633 LOC | Single `AgentMemory` orchestrator in `memory/memory.go` + processor pipelines + `memory.Option` API (history, recall, compaction, compression all configured through one `WithMemory` entry point) | Tie |
...
| **Per-turn compression**        | N/A (handled by Observational Memory)                                                               | `memory.WithCompress(fn, threshold)` — LLM-driven in-flight summarization without persistence dependency | Oasis |
